...
class Document extends DocumentManagerAppModel {
	...
	function readFolder($pathFolderNames) {
       ...
       if(!$folder->path)
       {
           $folder->path = getcwd()."/app/webroot/files";
       }
       // $folder->path = getcwd()."/app/webroot/files";
		$result = $folder->read(true, true, false);
		$folders = $result[0];
//var_dump($pathFolderNames);
//echo $pathFolderNames." the path to files<br />";
//        print_r(Configure::read('DocumentManager.excludeRootFiles'));
	...
	function saveFile($tmpName, $pathFolderNames, $fileName, $userId, $comments = null) {
		if (file_exists($path = $this->getFullPath($pathFolderNames, $fileName))) {
			return __d("document_manager", "A file with this name already exists.");
		}
		//$userId = $this->Auth->user('id');
       // echo $path." the path";die;
        $userId = 1; // this is a hack
		if (move_uploaded_file($tmpName, $path)) {
			...
			return __d("document_manager", "The file could not be saved.");
		}
	}
	...
	function saveDocument($data, $pathFolderNames = array(), $userId, $headers = array()) {
		if (substr($data['file']['name'], 0, 1) == '.') {
			return __d("document_manager", "The name of a file can not start with a period.");
			...
			if (!empty($headers['Content-Length']) && (int) $headers['Content-Length'] > ($max = $this->phpMaxUploadSize()) * 1024 * 1024) {
				...
				return __d("document_manager", "Files over %d Mb can not be uploaded.", $max);
			} else {
				return __d("document_manager", "The file could not be uploaded. Try Again.");
			}
		}
		return null;
	}
		...
		if (!file_exists($oldPath = $this->getFullPath($pathFolderNames, $oldName))) { // File doesn't exist
			...
			if (file_exists($newPath = $this->getFullPath($pathFolderNames, $newName))) {
				...
				if ($oldName != $newName) { // If same names, no problem
					return array(
						'error' => __d("document_manager", "A file with this name already exists."),
						'remove' => false,
					);
				}
			} else {
	...
	function createSubFolder($pathFolderNames, $folderName) {
		...
		if ($folderName == '') {
			return __d("document_manager", "Please provide a file name.");
		} else {
			$folderPath = $this->getFullPath($pathFolderNames, $folderName);
			if (is_dir($folderPath)) {
				return __d("document_manager", "This file already exists.");
			} else if (!mkdir($folderPath)) {
				return __d("document_manager", "The folder could not be created. Please check the name or permissions of the root directory.");
			} else {
