...
class DocumentsController extends DocumentManagerAppController {
	...
	public function create_subfolder() {
		if (substr($this->request->data('folderName'), 0, 1) == '.') {
			//$this->Session->setFlash(__d("document_manager", "The name of a folder can not begin with a period."));
            $this->Flash->error(__("The name of a folder can not begin with a period."));
		} else {
	...
	public function delete_file() {
		...
		if ($this->hasAdminRights() || $this->fileBelongsToUser($document['Document']['user_id'])) {
			$error = $this->Document->deleteFile($path) ?
				null : __d("document_manager", "The file could not be deleted.");
		} else {
			$error = __d("document_manager", "This file does not belong to you.");
		}
	...
	public function delete_folder() {
		...
		if ($this->hasAdminRights() || $this->folderBelongsToUser($path)) {
			$error = $this->Document->deleteFolder($path) ? null : __d("document_manager", "The folder could not be deleted.");
		} else {
			$error = __d("document_manager", "This folder contains files that do not belong to you.");
		}
